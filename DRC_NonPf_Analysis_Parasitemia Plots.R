...
#### blinded file path
setwd(".../DRC Non-falciparum Project/Analysis/Dataset_AD/Results")
...
#updating labels to exponential format so they fit better for figure sizing
...
dplot_PfbyPm<-ggplot(PfvsPfPm_active)+
  ...
  scale_x_log10(expression("Estimated Pf parasite density (p/"*mu*"L)"))
plot(dplot_PfbyPm)
...
PfbyPm_overlay<-ggplot(PfvsPfPm_active, aes(x=Pd_Pf, bins=10))+
  ...
  scale_x_log10(breaks=breaks_log(n=8), labels=trans_format("log10", math_format(10^.x)))+
  geom_density(aes(fill=factor(Pf_Species)), col='black', lwd=0.6, alpha=0.4)+
  xlab(expression("Estimated Pf parasite density (p/"*mu*"L)",))
...
PfbyPm_overlay<-ggplot(PfvsPfPm_active, aes(x=Pd_Pf, fill=Pf_Species, bins=10))+
  ...
  #scale_x_log10(name="", breaks=c(.01,.1,1,10, 100, 1000, 10000, 100000, 1000000),labels=xlabels_manual)+
  scale_x_log10(breaks=breaks_log(n=8), labels=trans_format("log10", math_format(10^.x)))+
  geom_density(aes(fill=factor(Pf_Species)), col='black', lwd=0.3, alpha=0.2)+
...
PfbyPm_overlay_fig <- PfbyPm_overlay + guides(fill=guide_legend(title="")) +
  ...
  theme_bw() + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
                     ...
                     text=element_text(size=5), legend.position = c(0.87, 0.86))
...
ggplot(PfvsPfPm_active) +
  ...
  scale_x_log10(expression("Estimated Pf parasite density (p/"*mu*"L)"), breaks=c(.01,.1,1,10, 100, 1000, 10000, 100000, 1000000),labels=xlabels_manual)
...
dplot_PfbyPo<-ggplot(PfvsPfPo_active)+
  geom_density(aes(x = Pd_Pf, fill = Pf_Species), alpha=0.5)+
  scale_x_log10(expression("Estimated Parasite Density (p/"*mu*"L)"))
plot(dplot_PfbyPo)
...
AggregatePlot_Pds <- gridExtra::grid.arrange(PfbyPm_overlay_fig2, PfbyPo_overlay_fig2, 
...
ggsave("Aggregate_Act_Pass_fig5_5Sep23.pdf", AggregatePlot_Pds, width = 180, height = 90, units = "mm") 

ggsave("Aggregate_Act_Pass_fig5_5Sep23_onecol.pdf", AggregatePlot_Pds, width = 90, height = 100, units = "mm") 
